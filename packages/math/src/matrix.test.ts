...
describe('@bothrs/math ~ Matrix', () => {
  ...
  it('Should correctly multiply matrices', () => {
    ...
    expect(multiplyMatrices(translationMatrix, scalingMatrix)).toEqual([
      [0.5, 0, 10],
      [0, 0.5, 10],
      [0, 0, 1],
    ])
  ...
  it('Should correctly generate an SVG transform matrix string', () => {
    ...
    expect(getSVGTransformMatrix(translationMatrix)).toEqual(
  ...
  it('Should correctly generate an transform matrix', () => {
    ...
    expect(transformMatrix).toEqual(
      'matrix(-0.2240368080645851, 0.4469983318002789, -0.4469983318002789, -0.2240368080645851, 10, 10)'
    )
  })
})
